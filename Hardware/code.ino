...
void handleRoot() {
  String html = "<html><body><h1>ENIG IAS PROJECT</h1>";
...
void handlePhoto() {
 ...
 camera_fb_t * fb = NULL;
  // Instead, directly take a photo and send it
 fb = esp_camera_fb_get(); // Capture image from camera
  if (!fb) {
    server.send(404, "text/plain", "Camera capture failed"); // Adjusted error handling
    return;
  }
